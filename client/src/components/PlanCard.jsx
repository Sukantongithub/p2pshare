  ...
  return (
    <div
      className={`rounded-lg shadow-lg p-6 text-center transition transform hover:scale-105 ${
        isPremium
          ? "bg-linear-to-br from-blue-600 to-blue-800 text-white ring-2 ring-blue-400"
    ...
    >
      ...
      <p
        className={`text-lg font-semibold mb-4 ${
          !isPremium && "text-gray-200"
        }`}
      >
        {plan.price === 0 ? "FREE" : `$${plan.price}/month`}
      </p>
      {error && (
        <p className="text-red-400 text-sm mb-4 bg-red-500/20 p-2 rounded">
          {error}
        </p>
      )}
      <button
