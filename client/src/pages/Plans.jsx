        ...
        {currentSub && (
          <div className="bg-blue-500/20 border border-blue-400/50 text-blue-200 px-6 py-4 rounded-lg mb-8">
            <p className="font-semibold">
              Current Plan:{" "}
              <span className="uppercase">{currentSub.currentPlan}</span>
            </p>
            <p className="text-sm">
              Storage: {(currentSub.storageUsed / 1024 ** 3).toFixed(2)}GB /{" "}
              {(currentSub.storageLimit / 1024 ** 3).toFixed(2)}GB (
              {currentSub.storagePercentage}%)
            </p>
            {currentSub.subscriptionExpiry && (
              <p className="text-sm">
                Expires:{" "}
                {new Date(currentSub.subscriptionExpiry).toLocaleDateString()}
              </p>
            )}
          </div>
        )}
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    fetchCurrentSubscription();
                    alert(
                      "Subscription cancelled. You will revert to free plan."
                    );
                  } catch (err) {
